<template>
  <v-card flat class="pb-8">
    <v-subheader class="px-0 py-3">
      <v-card-text class="text-color text-subtitle-1 pa-0 py-2">{{ $t('githubConnector.admin.label.repositories.placeholder') }}</v-card-text>
      <v-spacer />
      <v-card
        width="220"
        max-width="100%"
...
export default {
  ...
  computed: {
    ...
    repositoriesHeaders() {
      return [
        {text: this.$t('githubConnector.webhook.details.repository'), value: 'title', align: 'start', width: '20%' , class: 'dark-grey-color text-font-size ps-0',},
        {text: this.$t('githubConnector.webhook.details.description'), value: 'description', align: 'start', width: '60%', class: 'dark-grey-color text-font-size'},
        {text: this.$t('githubConnector.webhook.details.status'), value: 'enabled', align: 'center', width: '20%', class: 'dark-grey-color text-font-size'},];
    },
  },
  watch: {
    keyword() {
  ...
  methods: {
    retrieveHookRepositories() {
      ...
      return this.$githubConnectorService.getWebHookRepos(this.organizationId, page, itemsPerPage, this.keyword)
        .then(data => {
          this.repositories.push(...data.remoteRepositories);
          if (data.remoteRepositories.length <= itemsPerPage) {
            return this.$githubConnectorService.getWebHookRepos(this.organizationId, page + 1, itemsPerPage, this.keyword)
              .then(nextData => {
